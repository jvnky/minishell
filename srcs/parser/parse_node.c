...
/*   Updated: 2023/10/19 06:59:21 by ychair           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	***copy_parse_for_node(t_data *data, int i, int j)
{
	...
	dst = malloc(sizeof(char **) * (data->parse_i + 2));
	if (!dst)
		return (NULL);
	//dst[data->parse_i + 1] = '\0';
	dst[data->parse_i + 2] = NULL;
	while (data->parse[++i])
	{
		j = 0;
		while (data->parse[i][j])
			j++;
		dst[i] = malloc(sizeof(char *) * (j));
		if (!dst[i])
			return (NULL);
		dst[i][j] = NULL;
		j = -1;
