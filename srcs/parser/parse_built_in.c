...
static int check_env(t_args *args, t_data *data)
{
	...
	{
		if (add_command(data, "export") == 0)
			return (0);
		args->i += 6;
	}
	else if (ft_strcmpr(args, "unset") != 0)
	{
		if (add_command(data, "unset") == 0)
			return (0);
		args->i += 5;
	}
	else if (ft_strcmpr(args, "env") != 0)
	{
		if (add_command(data, "env") == 0)
			return (0);
		args->i += 3;
	}
	return (1);
...
int parse_built_in(t_args *args, t_data *data)
{
	...
	{
		if (add_command(data, "cd") == 0)
			return (0);
		args->i += 2;
	}
	else if (ft_strcmpr(args, "pwd") != 0)
	{
		if (add_command(data, "pwd") == 0)
			return (0);
		args->i += 3;
	}
